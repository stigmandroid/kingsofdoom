 ...
 * ----------------------------------------------------------
 * Arquivo:
 * app/api/internal/clash/raid-weekends/route.ts
 *
 * Responsabilidade:
 * Disponibilizar os Raid Weekends dos clãs monitorados ao
 * ambiente local através do gateway privado hospedado na VPS.
 *
 * A rota:
 *
 ...
 * • retorna somente os itens de Raid Weekend.
 *
 * Autor:
 * stigmandroid
 *
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error(
      "[Kings of Doom] Gateway de Raid Weekend não configurado:",
      error,
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Gateway de Raid Weekend não configurado.",
      },
      {
        status: 500,
      },
  ...
  const limit = Math.max(1, Math.min(100, Number(limitRaw ?? "3") || 3));
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[Kings of Doom] Erro ao consultar Raid Weekend:", {
        clanSlug,
        clanTag,
      ...
      return NextResponse.json(
        {
          ...
          error:
            error?.message ??
            error?.reason ??
            "Não foi possível consultar Raid Weekend.",
        },
        {
          status: response.status,
        },
    ...
    const data = (await response.json()) as ClashRaidWeekendResponse;
    ...
    console.error("[Kings of Doom] Falha ao consultar Raid Weekend:", {
      clanSlug,
      clanTag,
      error,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Não foi possível consultar Raid Weekend.",
      },
      {
        status: 500,
      },
