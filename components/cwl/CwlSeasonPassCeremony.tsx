 ...
 * ----------------------------------------------------------
 * Arquivo:
 * components/cwl/CwlSeasonPassCeremony.tsx
 *
 * Responsabilidade:
 * Executar a experiência cinematográfica oficial
 * de revelação do Passe de Temporada.
 *
 * Sequência:
 *
 ...
 * - recebe do evento o vencedor oficial já revelado pela API;
 * - a animação é exclusivamente visual.
 *
 * Autor:
 * stigmandroid
 *
 ...
 *
 * Status:
 * ✅ Simulação revisada e pronta para servir de base à integração real
 * ==========================================================
 */
 ...
 */
export type SeasonPassCeremonyPlayer = {
  tag: string;
  name: string;
...
/**
 * Cerimônia visual oficial do Passe de Temporada.
 */
export default function CwlSeasonPassCeremony({
  ...
  /**
   * Índice do vencedor oficial da execução atual.
   *
   * Existe apenas em memória e não representa
   * o resultado oficial recebido do servidor.
   */
  const winnerIndexRef = useRef<number | null>(null);

  /**
   * Garante que a trava visual do vencedor aconteça
   * apenas uma vez após o momento definido na timeline.
   *
   * Evita chamadas repetidas de setActiveIndex()
   * nos frames seguintes ao lock.
   */
  ...
  function startSimulation(): void {
    ...
    const officialWinnerIndex = players.findIndex(
      (player) => player.tag === winner.tag,
    );

    if (officialWinnerIndex < 0) {
      console.error(
        "[Kings of Doom] Vencedor oficial não encontrado na lista elegível.",
        winner,
      );
      return;
    }
  ...
  function startCeremony(): void {
    ...
    const animate = (timestamp: number): void => {
      ...
      /**
       * Alterna os participantes enquanto o sorteio está ativo.
       *
       * O intervalo entre as trocas cresce quadraticamente,
       * criando a desaceleração visual antes da trava do vencedor.
       */
    ...
    <div className="flex flex-col justify-between gap-6 p-5 sm:flex-row sm:items-center sm:p-8">
      <div>
        ...
        <h3 className="mt-2 text-xl font-black text-white sm:text-2xl">
          Cerimônia do Passe de Temporada
        </h3>

        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
          O vencedor já foi definido e persistido pelo servidor. Esta etapa
          executa apenas a revelação cinematográfica.
        </p>

        <p className="mt-3 text-xs font-semibold text-slate-600">
          {playersCount} participantes do sorteio
        </p>
      </div>
...
function CeremonyScene({
  ...
  elapsed: number;
  activePlayer?: SeasonPassCeremonyPlayer;
  winner?: SeasonPassCeremonyPlayer;
}) {
  /**
...
function WinnerIdentity({
  ...
  player?: SeasonPassCeremonyPlayer;
  locked: boolean;
  tagVisible: boolean;
  finalLayout: boolean;
}) {
